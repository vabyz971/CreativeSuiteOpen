...
//! supprimer). Les commandes filtres/masques/visibilité/suppression
//! partent au worker via le channel (non bloquant) ; la sélection et
//! le renommage remontent au layout via [`LayerItemAction`].

use super::PhotoLayerInfo;
...
/// Action remontée au layout (sélection + renommage : état UI détenu
/// par l'app ; tout le reste part direct au worker dans ce fichier).
...
///
/// Toute icône passe par `CygnusIcon`. Les envois channel échoués
/// (worker arrêté) sont silencieusement ignorés : l'UI ne panique
/// jamais.
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                if icon_button(ui, CygnusIcon::Delete, Some("Supprimer ce calque")).clicked() {
                    let _ = engine_tx.send(PhotoEngineCommand::DeleteLayer(layer.id));
                }
            });
        });
                ...
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ...
                        for mask in &layer.masks {
                            draw_sub_layer(
                                ui, &theme, layer.id, mask.id, &mask.name, false, engine_tx,
                            );
                        }
                    });
                });
...
/// descendre / supprimer. Les commandes partent direct au worker.
...
mod tests {
    use super::*;
    use crate::ui::layers::PhotoLayerKind;
    ...
    fn item_renders_without_panic() {
        ...
        ctx.run_ui(egui::RawInput::default(), |ui| {
            egui::CentralPanel::default().show(ui, |ui| {
                let actions = draw_photo_layer_item(ui, &layer, true, &mut rename, &tx);
                ...
                for kind in [
                    ...
                    let _ = draw_photo_layer_item(ui, &probing, false, &mut rename, &tx);
                }
            });
        })
    ...
    #[test]
    fn item_without_interaction_sends_nothing() {
        let ctx = egui::Context::default();
        ui_kit::theme::setup_fonts(&ctx);
        let layer = fixture_layer();
        let (tx, rx) = channel();
        ...
        ctx.run_ui(egui::RawInput::default(), |ui| {
            egui::CentralPanel::default().show(ui, |ui| {
                draw_photo_layer_item(ui, &layer, false, &mut rename, &tx);
            });
        })
        .drop_without_applying_deltas();
