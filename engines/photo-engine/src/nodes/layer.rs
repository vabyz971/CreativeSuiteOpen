...
pub fn apply_effect(
    ...
    // Tenter le GPU (bandes, opérations ponctuelles exactes)
    if let Some(out) =
        crate::gpu::apply_blend_gpu(base, top, opacity, mode_id(blend_mode), offset_x, offset_y)
    {
        return out;
    }
